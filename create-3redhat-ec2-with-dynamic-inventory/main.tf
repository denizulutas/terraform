...
resource "aws_instance" "managed_nodes" {
  ...
  key_name = "********"  # write your keypem without extention.
  vpc_security_group_ids = [aws_security_group.tf-sec-gr.id]
  tags = {
...
resource "null_resource" "config" {
  ...
  connection {
    ...
    type = "ssh"
    user = "ec2-user"
    private_key = file("~/.ssh/*******.pem") # fill out your pemkey path.
  }
  ...
  provisioner "file" {
    source = "~/.ssh/********.pem"  # fill out your pemkey path.
    destination = "/home/ec2-user/*******.pem"
  }
